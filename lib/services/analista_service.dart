...
class AnalistaService {
  ...
  Future<List<Analista>?> obterAnalistasPorEquipe(String idEquipe) async {
    try {
      QuerySnapshot snapshot = await _db
          .collection('analista')
          .where('id_equipe', isEqualTo: idEquipe)
  ...
  Future<void> editarDadosAnalista(
    String idAnalista,
    String nome,
    String email,
  ) async {
  ...
  Future<void> bloquearAnalista(
    ...
    try {
      await _db.collection('analista').doc(idAnalista).update({
        ...
        'descricao_status':
            descricaoStatus, // provém de uma lista de descrições pré-definidas no front-end
      });
    } catch (error) {
      throw "Erro ao editar cadastro de Analista: ${error.toString()}";
    }
  ...
  Future<void> excluirAnalista(String id) async {
    try {
      ...
      List<Acao>? acoes = await acaoService.obterAcoesTotaisPorAnalista(
        id,
        null,
      );
      if (acoes != null) {
  ...
  Future<String?> criarAnalista(Analista analista) async {
    try {
      QuerySnapshot snapshotAnalista = await _db
          .collection('analista')
          .where('email', isEqualTo: analista.email)
          .get();
      QuerySnapshot snapshotGestor = await _db
          .collection('gestor')
          .where('email', isEqualTo: analista.email)
          .get();
